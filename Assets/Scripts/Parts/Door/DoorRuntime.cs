...
{
    [Header("Authoring")]
    [SerializeField] private DoorAuthoring authoring;
    ...
    [Tooltip("If false, trigger colliders will not block door closing.")]
    [SerializeField] private bool triggersBlockClosing = false;

    [Tooltip("Ignore colliders that belong to this door object and its children.")]
    [SerializeField] private bool ignoreOwnColliders = true;

    [Tooltip("Ignore colliders under the same Boat root. Usually false; use the layer mask to avoid Hull false positives.")]
    ...
    public DoorAuthoring Authoring => authoring;
    ...
    {
        if (authoring == null)
            authoring = GetComponent<DoorAuthoring>();
    }
    ...
    private void Awake()
    {
        ...
        if (authoring == null)
        {
            Debug.LogError("[DoorRuntime] Missing DoorAuthoring.", this);
            enabled = false;
            return;
        }
    ...
    public bool SetOpen(bool open)
    {
        ...
        if (!open && !CanClose(out Collider2D blocker, out string reason))
        {
            ...
            {
                Debug.LogWarning(
                    $"[DoorRuntime:{name}] Close denied. {reason} " +
    ...
    [ContextMenu("Refresh Door Presentation")]
    ...
    private void OnDrawGizmosSelected()
    {
        ...
        DrawColliderGizmo(
            authoring.BlockingCollider,
            new Color(1f, 0.65f, 0.15f, 0.25f),
